unsigned short hs_binary_floatToHalf (float f) {
...
int hs_binary_halfToFloatRep (unsigned short c) {
  int s = (c >> 15) & 0x001;
  int e = (c >> 10) & 0x01f;
  int m =         c & 0x3ff;
  if (e == 0) {
...
float hs_binary_halfToFloat (unsigned short c) {
  union { float d; unsigned int i; } u;
  u.i = hs_binary_halfToFloatRep(c);
  return u.d;
}
